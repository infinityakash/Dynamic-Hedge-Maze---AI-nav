...
class Maze:
	...
	def add_can_description(self, f_out, coords):
		...
		for i in coords:
			x, y = i
			can_count += 1
			self.cans["can"+str(can_count)] = {}
			self.cans["can"+str(can_count)] = [x,y]
	...
	def generate_maze(self, grid_dimension, n_obstacles, seed, scale=0.5):
		...
		count = 1
		root_path = "/home/cse-571/catkin_ws/src/search"
		while(count <= n_obstacles):
			...
			if(flag == 0 and ((x+scale) <= grid_dimension*scale) and ((x, y, x+scale, y) not in blocked_edges)):
				...
				count += 1
			elif(flag == 2 and ((y+scale) <= grid_dimension*scale) and ((x+scale) <= grid_dimension*scale) and ((x, y, x+scale, y+scale) not in blocked_edges)and ((x+scale, y, x, y+scale) not in blocked_edges)):
				print('flag 2')
				blocked_edges.add((x, y, x+scale , y+scale))
				blocked_edges.add((x+scale, y ,x, y+scale))
				#Adding obstacle in the middle with some offset value of the edge to be blocked
				offset = np.random.uniform(0, 0.07*scale)
				coords.append((x+scale/2 , y+scale/2))
				f_out.write('<model name=\'can{}\'>\n'.format(count))
				self.add_can(f_out, x+0.25, y+0.25)
				count += 1
		...
		with open(root_path + '/cans.json','w') as fp:
			json.dump(self.cans,fp)
		f_out.write('</world>\n</sdf>')
		f_out.close()
